...
WGCNA_matrix1 <- read.table("Merge_RNA_seq_FPKM.txt",header=T,
                           comment.char = "",
                           check.names=F)
...
TCGA_BRCA_RNA_seq <- TCGA_BRCA_RNA_seq + 1
TCGA_BRCA_RNA_seq <- log2(TCGA_BRCA_RNA_seq)
...
for (i in 1:n) {
  if (datclinical[i, "ER"] == "Negative" && datclinical[i, "PR"] == "Negative" && datclinical[i, "HER2"] == "Negative") {
    trainDt[i, 1] <- 1
  } else {
    trainDt[i, 1] <- 0
  }
...
modTraitP = corPvalueStudent(modTraitCor, nSamples)
MEblue=modTraitCor[which(row.names(modTraitCor)=='MEblue'),]
head(MEblue[order(-MEblue)],50)
